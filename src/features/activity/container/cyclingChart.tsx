...
const CyclingChart = () => {
  const chartData = {
    labels: jsonData.activities[1].dailyActivity?.map((item, index) => item.name),
    datasets: [
      {
        label: 'Heart Rate (max)',
        data: jsonData.activities[1].dailyActivity?.map((item, index) => item.heart_rate.max),
      ...
      },
      {
        ...
        data: jsonData.activities[1].dailyActivity?.map((item, index) => item.heart_rate.average),
